  ...
  const model = {
    ...
    async updateMany({ where, data }) {
      ...
      for (const [key, row] of rows) {
        ...
        rows.set(key, { ...row, ...data }); count += 1;
      }
      return { count };
    },
    async create({ data }) { rows.set(data.tokenHash, { ...data, claimedAt: null, usedAt: null, supersededAt: null }); return rows.get(data.tokenHash); },
    async findUnique({ where }) { return rows.get(where.tokenHash) || null; },
    async update({ where, data }) { const row = { ...rows.get(where.tokenHash), ...data }; rows.set(where.tokenHash, row); return row; },
...
test("verification tokens are stored hashed and can be claimed only once", async () => {
  ...
  const token = await store.create({ payload: { business: { name: "Safe Co" } }, ownerEmail: "owner@example.com", businessName: "Safe Co" });
...
test("a claimed verification can be retained for safe recovery", async () => {
  ...
  const store = createPendingSignupVerificationStore({ prisma, minimumTtlMs: 1000 });
  const token = await store.create({ payload: { state: "submitted" }, ownerEmail: "owner@example.com", businessName: "Safe Co" });
  const claimed = await store.claim(token);
  const retained = await store.retainForRecovery(claimed.tokenHash, { record: claimed.record, payload: { state: "verified" }, reviewReasons: ["provider_retry"] });
