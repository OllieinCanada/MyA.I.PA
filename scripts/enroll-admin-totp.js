...
const serviceId = String(process.argv.find((arg) => arg.startsWith("--service-id="))?.slice(13) || process.env.RENDER_SERVICE_ID || "").trim();
...
async function main() {
  ...
  const uri = `otpauth://totp/${encodeURIComponent("My AI PA Admin")}?secret=${secret}&issuer=${encodeURIComponent("My AI PA")}&algorithm=SHA1&digits=6&period=30`;
