...
if (!service) {
  ...
  expectEnvValue(service, "OPERATIONAL_STATE_BACKEND", "database");
  ...
  expectUniqueEnvValue(service, "TELEGRAM_GUARDED_ACTIONS_ENABLED", "true");
  expectUniqueEnvValue(service, "TELEGRAM_WEBHOOK_URL", "https://api.myaipa.ca/api/webhooks/telegram/actions");
  expectNonSyncedSecret(service, "TELEGRAM_ADMIN_USER_ID");
  for (const key of ["TELEGRAM_WEBHOOK_SECRET", "TELEGRAM_ACTION_SIGNING_SECRET"]) {
    if (!hasEnv(service, key, (item) => item.generateValue === true)) fail(`${key} should be generated by Render`);
  }
  expectUniqueEnvValue(service, "INCIDENT_SAFE_AUTO_REPAIR_ENABLED", "true");
  expectUniqueEnvValue(service, "INCIDENT_CODE_REPAIR_ENABLED", "true");
  ...
  expectUniqueEnvValue(service, "INCIDENT_CONTAINMENT_NOTICE_AFTER_MS", "120000");
  expectUniqueEnvValue(service, "INCIDENT_SLO_CAPTURE_MS", "10000");
  expectUniqueEnvValue(service, "INCIDENT_SLO_CODEX_DISPATCH_MS", "60000");
  expectUniqueEnvValue(service, "INCIDENT_SLO_CONTAINMENT_MS", "180000");
  expectUniqueEnvValue(service, "INCIDENT_SLO_DRAFT_REPAIR_MS", "1800000");
  expectUniqueEnvValue(service, "INCIDENT_SLO_MAXIMUM_UNACKNOWLEDGED_MS", "3600000");
  ...
  for (const key of ["GITHUB_INCIDENT_REPAIR_APP_ID", "GITHUB_INCIDENT_REPAIR_INSTALLATION_ID", "GITHUB_INCIDENT_REPAIR_APP_PRIVATE_KEY"]) {
    expectNonSyncedSecret(service, key);
  }
  ...
  for (const key of ["FIELD_SERVICE_TOKEN_ENCRYPTION_KEY", "FIELD_SERVICE_OAUTH_STATE_SECRET"]) {
    if (!hasEnv(service, key, (item) => item.generateValue === true)) fail(`${key} should be generated by Render`);
  }
  if (!hasEnv(service, "SMS_SUPPRESSION_API_KEY", (item) => item.generateValue === true)) {
    fail("SMS_SUPPRESSION_API_KEY should be generated by Render");
  }
  for (const key of [
