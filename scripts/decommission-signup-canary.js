...
async function main() {
  ...
  const otherPhoneUses = phones.filter((item) => String(item?.id || "") !== String(phone.id || "") && String(item?.assistantId || item?.assistant?.id || "") === assistantId);
  if (otherPhoneUses.length) throw new Error("The canary assistant is attached to another phone; refusing deletion.");
  ...
  const audit = `${JSON.stringify({ ...summary, removedAt: new Date().toISOString(), phoneId: phone.id, assistantId, twilioSidHash: twilioNumber?.sid ? crypto.createHash("sha256").update(twilioNumber.sid).digest("hex") : "" }, null, 2)}\n`;
  fs.writeFileSync(path.join(outputDir, `signup-canary-2415-${stamp}.json`), audit, { flag: "wx" });
