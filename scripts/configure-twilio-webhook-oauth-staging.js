...
async function main() {
  ...
  if (hasFlag("replay")) {
    if (flagValue("confirm-replay") !== REPLAY_CONFIRMATION) throw new Error(`Replaying requires --confirm-replay=${REPLAY_CONFIRMATION}.`);
    const result = await replayEvent(env);
