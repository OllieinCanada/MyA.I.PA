...
describe("intuitive homepage journey", () => {
  ...
  test("restores the three-part animated proof and starts audio only when pressed", async () => {
    ...
    expect(container.textContent).toMatch(/Hi, thanks for calling Tim’s Electrical\. We handle residential, commercial, and industrial electrical work\. Do you need maintenance, a new installation, or a repair today\?/i);
    ...
    expect(container.textContent).toMatch(/Next action: pricing and scheduling follow-up/i);
    expect(container.textContent).toMatch(/We'll pass this information and we'll get back to you for pricing and scheduling/i);
    ...
    expect(container.textContent).toMatch(/Natural conversation and Answers to FAQs/i);
    expect(container.textContent).toMatch(/job and caller information collected/i);
    expect(container.textContent).toMatch(/The details your team needs—neatly organized/i);
    expect(container.textContent).toMatch(/preferred call back number, the best time to reach them, and any direct message they want to leave/i);
    expect(container.textContent).toMatch(/Reason for the call, work requested, service address, and city/i);
    expect(container.textContent).toMatch(/Preferred start date, availability, and any urgent concerns/i);
    expect(container.textContent).not.toMatch(/urgent non-emergency concern/i);
    expect(container.textContent).toMatch(/Services you provide, your service area and answers to common questions shape the receptionists' dialogue/i);
    expect(container.textContent).toMatch(/See your call details on your dashboard and follow up texts on your phone in order to improve your receptionist response/i);
    expect(container.textContent).toMatch(/confirmation the customer received on your dashboard/i);
    expect(container.querySelector("#why-it-matters").textContent).toMatch(/Your customer has a problem/i);
    expect(container.querySelector("#why-it-matters .simple-red-marker").textContent).toBe("now");
    expect(container.querySelector("#why-it-matters").textContent).toMatch(/Assistant answers the call/i);
    expect(container.querySelector("#why-it-matters .simple-decision-mark.negative").textContent).toBe("×");
    expect(container.querySelector("#why-it-matters .simple-decision-mark.positive").textContent).toBe("✓");
    expect(container.textContent).toMatch(/reason for service request/i);
